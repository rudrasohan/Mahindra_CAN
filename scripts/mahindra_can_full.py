...
def callback_joy(data):
    ...
    if data.buttons[2]:                      #X->steering  //confirm
        ...
        datstr-=5
    elif data.buttons[1]:   		     #B->steering  //confirm
        datstr+=5
    elif (data.axes[4]>0):                  #Right-Stick -> up->on down->off Headlight
        H=1
    elif (data.axes[4]<0):
        H=0
    elif data.axes[1]>0.8:		    #Gear Left-Stick up->forward down->rev
        gear_cmd="F"
    elif data.axes[1]<-0.8:
        gear_cmd="B"
    elif data.buttons[9]:			#Gear press-left stick Neutral
        gear_cmd="N"
    elif data.axes[6]==-1:			#D-Pad/cross key   indicator Left<->right
        Ind="L"
    elif data.axes[6]==1:
        Ind="R"
    elif data.axes[7]==-1:			#Dpad/cross key 5 off-indicator
        Ind="Off"
    elif data.buttons[5]:                       #horn RB
        RB_FLAG = 1
